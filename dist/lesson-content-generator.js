...
exports.generateEnhancedLessonContent = exports.generateLessonContent = exports.VIDEO_CONTENT_MAP = void 0;
// Lesson content generator based on video resources and lesson structure
const lesson_structure_1 = require("./lesson-structure");
// Complete video mapping based on ALL 63 videos from src/resources/videos
exports.VIDEO_CONTENT_MAP = {
...
const generateLessonContent = (lessonId, lessonType) => {
    const questions = [];
...
const generateEnhancedLessonContent = (lessonId, lessonType) => {
    const questions = [];
    const videoCategory = getVideoCategoryFromLessonId(lessonId);
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
...
exports.generateEnhancedLessonContent = generateEnhancedLessonContent;
// Helper function to extract category from lesson ID
const getCategoryFromLessonId = (lessonId) => {
    if (lessonId.includes('01_01'))
        return 'greetings';
    if (lessonId.includes('01_02'))
        return 'family';
    if (lessonId.includes('02_01'))
        return 'emotions';
    if (lessonId.includes('02_02'))
        return 'emotions';
    if (lessonId.includes('03_01'))
        return 'animals';
    if (lessonId.includes('03_02'))
        return 'weather';
    if (lessonId.includes('04_01'))
        return 'numbers';
    if (lessonId.includes('04_02'))
        return 'shapes';
    if (lessonId.includes('05_01'))
        return 'food';
    if (lessonId.includes('05_02'))
        return 'objects';
    if (lessonId.includes('06_01'))
        return 'transportation';
    if (lessonId.includes('06_02'))
        return 'activities';
    if (lessonId.includes('07_01'))
        return 'work';
    if (lessonId.includes('07_02'))
        return 'work';
    return 'general';
};
// Helper function to get video category
const getVideoCategoryFromLessonId = (lessonId) => {
    // Map lesson IDs to specific video categories based on actual lesson structure
    if (lessonId.includes('08_01'))
        return 'complexEmotions';
    if (lessonId.includes('08_02'))
        return 'specialEmotions';
    if (lessonId.includes('09_01'))
        return 'studyObjects';
    if (lessonId.includes('09_02'))
        return 'personalItems';
    if (lessonId.includes('10_01'))
        return 'nature';
    if (lessonId.includes('10_02'))
        return 'environment';
    ...
    if (lessonId.includes('02_01'))
        return 'emotions';
    if (lessonId.includes('02_02'))
        return 'emotions'; // Chapter 4: Cảm xúc nâng cao
    if (lessonId.includes('03_01'))
        return 'animals';
    if (lessonId.includes('03_02'))
        return 'weather';
...
const generateDiscoverQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    ...
    if (selected.length >= 2) {
        ...
        firstTwoWords.forEach(([, data], idx) => {
            questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-content-${idx + 1}`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1A, `Từ mới: ${data.label}`, data.video, data.label));
        });
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (questionType === 'single') {
                ...
                questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-single-${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2A, `Video nào là từ '${data.label}'?`, data.video, data.label, distractors.map(d => d.label), distractors.map(d => d.video)));
            }
                ...
                questions.push({
                    ...
                    type: 'multiple',
                    category: lesson_structure_1.QuestionCategories.MULTIPLE.CAT_3A,
                ...
                questions.push({
                    ...
                    type: 'cloze_answer',
                    category: lesson_structure_1.QuestionCategories.CLOZE_ANSWER.CAT_4A,
    ...
    if (selected.length >= 5) {
        ...
        nextThreeWords.forEach(([, data], idx) => {
            questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-content-${idx + 3}`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1A, `Từ mới: ${data.label}`, data.video, data.label));
        });
        ...
        for (let i = 0; i < 3; i++) {
            ...
            if (questionType === 'single') {
                // Single: Hiển thị tiêu đề "Video nào là từ X", chọn 1 trong 2 video
                const distractors = otherWords.slice(0, 1).map(([, d]) => ({ label: d.label, video: d.video }));
                questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-single-${i + 4}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2A, `Video nào là từ '${data.label}'?`, data.video, data.label, distractors.map(d => d.label), distractors.map(d => d.video)));
            }
            else if (questionType === 'multiple_choice') {
                // Multiple Choice: Hiển thị 1 video, hỏi nghĩa của video đó, chọn đáp án đúng
                // Random số lượng đáp án: 2 hoặc 4
...
const generateVokabelQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    // Add content introduction with comprehensive vocabulary overview
    questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-intro`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1C, `Từ vựng ${getCategoryTitle(videoCategory)} - ${videoEntries.length} từ mới!`, videoEntries[0]?.[1]?.video || '', `Học ${videoEntries.length} từ vựng về ${getCategoryTitle(videoCategory)}`));
    ...
    for (let i = 0; i < maxQuestions; i++) {
        ...
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-vocab${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2B, `Từ vựng ${i + 1}/${maxQuestions}: Ký hiệu nào có nghĩa là '${videoData.label}'?`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    ...
    if (videoEntries.length > 1) {
        ...
        questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-vocab-summary`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1C, `Tóm tắt từ vựng: Bạn đã học ${maxQuestions} từ mới về ${getCategoryTitle(videoCategory)}!`, videoEntries[0]?.[1]?.video || '', `Các từ vựng đã học: ${allLabels}`));
    }
    return questions;
};
// Generate questions for Develop lesson type
const generateDevelopQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    ...
    for (let i = 0; i < Math.min(3, videoEntries.length); i++) {
        const [key, videoData] = videoEntries[i];
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-dev${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2C, `Phát triển kỹ năng: Chọn ký hiệu '${videoData.label}'!`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    return questions;
};
// Generate questions for Iconic lesson type
const generateIconicQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    ...
    for (let i = 0; i < Math.min(2, videoEntries.length); i++) {
        ...
        questions.push({
            ...
            type: 'single',
            category: lesson_structure_1.QuestionCategories.SINGLE.CAT_2G,
...
const getIconicEmoji = (videoCategory) => {
    const emojiMap = {
        greetings: '👋',
        family: '👨‍👩‍👧‍👦',
        emotions: '😊',
...
const generateDialogQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    ...
    for (let i = 0; i < Math.min(3, videoEntries.length); i++) {
        const [key, videoData] = videoEntries[i];
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-dialog${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2D, `Trong hội thoại: Ký hiệu '${videoData.label}' được dùng khi nào?`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    return questions;
};
// Generate questions for Training lesson type
const generateTrainingQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    if (videoEntries.length === 0) {
    ...
    for (let i = 0; i < selected.length; i++) {
        ...
        if (questionType === 'single') {
            // Single: Hiển thị tiêu đề "Video nào là từ X", chọn 1 trong 2 video
            const distractors = otherWords.slice(0, 1).map(([, d]) => ({ label: d.label, video: d.video }));
            questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-train-single-${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2F, `Luyện tập: Video nào là từ '${videoData.label}'?`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
        }
        else if (questionType === 'multiple_choice') {
            // Multiple Choice: Hiển thị 1 video, hỏi nghĩa của video đó, chọn đáp án đúng
            // Random số lượng đáp án: 2 hoặc 4
...
const generateReviewQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    ...
    for (let i = 0; i < Math.min(5, videoEntries.length); i++) {
        const [key, videoData] = videoEntries[i];
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-review${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2M, `Ôn tập: Chọn ký hiệu '${videoData.label}'!`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    return questions;
};
// Generate questions for Quiz lesson type - Enhanced with 4-5 new words per topic
const generateQuizQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    // Add quiz introduction with comprehensive overview
    questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-intro`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1A, `Kiểm tra toàn diện: ${getCategoryTitle(videoCategory)} - ${videoEntries.length} từ mới!`, videoEntries[0]?.[1]?.video || '', `Tổng cộng ${videoEntries.length} từ vựng về ${getCategoryTitle(videoCategory)}`));
    ...
    for (let i = 0; i < maxQuestions; i++) {
        const [key, videoData] = videoEntries[i];
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-quiz${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2N, `Từ ${i + 1}/${maxQuestions}: Ký hiệu nào có nghĩa là '${videoData.label}'?`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    // Add summary question if there are multiple videos
    if (videoEntries.length > 1) {
        const allLabels = videoEntries.map(([, data]) => data.label).join(', ');
        questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-summary`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1C, `Tóm tắt: Bạn đã học ${videoEntries.length} từ mới về ${getCategoryTitle(videoCategory)}!`, videoEntries[0]?.[1]?.video || '', `Các từ đã học: ${allLabels}`));
    }
    return questions;
};
// Generate questions for Info lesson type
const generateInfoQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    // Add informational content
    questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-info`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1C, `Thông tin bổ sung về ${getCategoryTitle(videoCategory)}`, videoEntries[0]?.[1]?.video || '', getCategoryTitle(videoCategory)));
    return questions;
};
// Generate questions for Explore lesson type
const generateExploreQuestions = (lessonId, videoCategory) => {
    const questions = [];
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
    const videoEntries = Object.entries(videos);
    ...
    for (let i = 0; i < Math.min(4, videoEntries.length); i++) {
        const [key, videoData] = videoEntries[i];
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-explore${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2K, `Khám phá: '${videoData.label}' có ý nghĩa gì?`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    return questions;
};
// Helper functions
const getCategoryTitle = (videoCategory) => {
    const titles = {
        greetings: 'chào hỏi',
        family: 'gia đình',
        emotions: 'cảm xúc',
...
const getRandomDistractors = (videoCategory, excludeKey, count) => {
    const videos = exports.VIDEO_CONTENT_MAP[videoCategory];
...
const generateEnhancedDiscoverQuestions = (lessonId, videoCategory, shuffledVideos) => {
    const questions = [];
    // Add comprehensive introduction with video preview
    questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-intro`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1A, `Khám phá ${getCategoryTitle(videoCategory)} - ${shuffledVideos.length} từ mới với video!`, shuffledVideos[0]?.[1]?.video || '', `Học ${shuffledVideos.length} từ vựng về ${getCategoryTitle(videoCategory)} với video minh họa`));
    ...
    for (let i = 0; i < maxQuestions; i++) {
        ...
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-video-meaning-${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2A, `Video ${i + 1}/${maxQuestions}: Ký hiệu này có nghĩa là gì?`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    ...
    for (let i = 0; i < Math.min(maxQuestions, 6); i++) {
        ...
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-meaning-video-${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2B, `Từ ${i + 1}/${Math.min(maxQuestions, 6)}: Chọn video đúng cho '${videoData.label}'!`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    ...
    if (shuffledVideos.length > 1) {
        ...
        questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-discovery-summary`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1C, `Tóm tắt khám phá: Bạn đã học ${shuffledVideos.length} từ mới về ${getCategoryTitle(videoCategory)}!`, shuffledVideos[0]?.[1]?.video || '', `Các từ đã khám phá: ${allLabels}`));
    }
    return questions;
};
// Enhanced Quiz Questions with shuffle and comprehensive coverage
const generateEnhancedQuizQuestions = (lessonId, videoCategory, shuffledVideos) => {
    const questions = [];
    // Add quiz introduction with comprehensive overview
    questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-quiz-intro`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1A, `Kiểm tra toàn diện: ${getCategoryTitle(videoCategory)} - ${shuffledVideos.length} từ mới!`, shuffledVideos[0]?.[1]?.video || '', `Tổng cộng ${shuffledVideos.length} từ vựng về ${getCategoryTitle(videoCategory)}`));
    ...
    for (let i = 0; i < maxQuestions; i++) {
        const [key, videoData] = shuffledVideos[i];
        const distractors = getRandomDistractors(videoCategory, key, 3);
        questions.push((0, lesson_structure_1.createVideoQuestion)(`${lessonId}-quiz-${i + 1}`, lesson_structure_1.QuestionCategories.SINGLE.CAT_2N, `Quiz ${i + 1}/${maxQuestions}: Ký hiệu nào có nghĩa là '${videoData.label}'?`, videoData.video, videoData.label, distractors.map(d => d.label), distractors.map(d => d.video)));
    }
    // Add summary question if there are multiple videos
    if (shuffledVideos.length > 1) {
        const allLabels = shuffledVideos.map(([, data]) => data.label).join(', ');
        questions.push((0, lesson_structure_1.createContentQuestion)(`${lessonId}-quiz-summary`, lesson_structure_1.QuestionCategories.CONTENT.CAT_1C, `Tóm tắt quiz: Bạn đã học ${shuffledVideos.length} từ mới về ${getCategoryTitle(videoCategory)}!`, shuffledVideos[0]?.[1]?.video || '', `Các từ đã học: ${allLabels}`));
    }
    return questions;
};
